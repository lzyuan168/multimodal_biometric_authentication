...
def main():
    ...
    train_dataset = FGECGVerificationDataset(DATA_PATH, 'train', 'enhanced', 'ecg_img', fg_transforms=fg_transforms, ecg_transforms=ecg_transforms)
    test_dataset = FGECGVerificationDataset(DATA_PATH, 'test', 'enhanced', 'ecg_img', fg_transforms=fg_transforms, ecg_transforms=ecg_transforms)
    ...
    wandb.init(
        ...
        name="siam_{}_run9(enh_veri_fg)_BN_L2".format(MODEL_NAME),
        config={
            "learning_rate": LR,
            "epochs": N_EPOCHS,
            "batch":BATCH_SIZE,
    ...
    for epoch in range(1, N_EPOCHS+1):
        ...
        if val_accuracy >= best_val_acc:
            ...
            model_checkpoint_path = os.path.join(MODEL_CKPT, "{}{}_epoch_{}.pth".format(MODEL_NAME,"_siam_enh_verification", epoch))
