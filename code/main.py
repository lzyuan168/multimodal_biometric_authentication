...
def main():
    ...
    mean = IMAGENET_DEFAULT_MEAN #this for ECG, for fingerprint use 0.5
    ...
    train_dataset = FingerprintDataset(DATA_PATH, 'train', 'binary', transforms=fg_transforms)
    test_dataset = FingerprintDataset(DATA_PATH, 'test', 'binary', transforms=fg_transforms)

    # train_dataset = ECGDataset(DATA_PATH, 'train', transforms=ecg_transforms)
    # test_dataset = ECGDataset(DATA_PATH, 'test', transforms=ecg_transforms)
    ...
    model = VGG16(inchannel=1).to(device)
    ...
    # wandb.login()
    wandb.init(
        project="Baseline_fg_experiment",
        entity="Zhiyuanthesis",
        name="fg_{}_binimg_BN_L2".format(MODEL_NAME),
        config={
            "learning_rate": LR,
            "epochs": N_EPOCHS,
            "batch":BATCH_SIZE,
    ...
    for epoch in range(1, N_EPOCHS+1):
        ...
        if val_accuracy >= best_val_acc:
            ...
            model_checkpoint_path = os.path.join(MODEL_CKPT, "{}{}_binary_epoch_{}.pth".format(MODEL_NAME,"_fg", epoch))
    ...
    # model_checkpoint_path = os.path.join(MODEL_CKPT, "{}{}.pth".format(MODEL_NAME,"_fg"))
    # torch.save( {"model_state_dict": model.state_dict()}, model_checkpoint_path)
