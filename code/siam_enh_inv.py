...
def main():
    ...
    train_dataset = FGECGDataset(DATA_PATH, 'train', 'enhanced_inv', 'ecg_img', fg_transforms=fg_transforms, ecg_transforms=ecg_transforms)
    test_dataset = FGECGDataset(DATA_PATH, 'test', 'enhanced_inv', 'ecg_img', fg_transforms=fg_transforms, ecg_transforms=ecg_transforms)

    # train_dataset = FGECGVerificationDataset(DATA_PATH, 'train', 'binary_inv', 'ecg_img', fg_transforms=fg_transforms, ecg_transforms=ecg_transforms)
    # test_dataset = FGECGVerificationDataset(DATA_PATH, 'test', 'binary_inv', 'ecg_img', fg_transforms=fg_transforms, ecg_transforms=ecg_transforms)
    ...
    wandb.init(
        ...
        name="siam_{}_run7(enh_inv_fg)_BN_L2".format(MODEL_NAME),
        config={
            "learning_rate": LR,
            "epochs": N_EPOCHS,
            "batch":BATCH_SIZE,
    ...
    for epoch in range(1, N_EPOCHS+1):
        ...
        if val_accuracy >= best_val_acc:
            ...
            model_checkpoint_path = os.path.join(MODEL_CKPT, "{}{}_epoch_{}.pth".format(MODEL_NAME,"_siam_enh_inv_fg", epoch))
